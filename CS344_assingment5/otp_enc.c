...
int main(int argc, char *argv[]) {
	...
	char textbuffer[NUM1], keybuffer[NUM1], cmbuffer[NUM2], encryptdatabuffer[NUM1];
	...
	sizeoftext = sizeoftext +2; // Include the size for terminating characters
	// Get the size of key file
	char * str = argv[2];
	sizeofkey = file_size(argv[2]);
    int check5;
	...
	sizeofkey = sizeofkey+ 2; // Include the size for terminating characters
	...
	while (index1 < sizeof(cmbuffer) - 1) {
	...
	int readdata;
	int index2 = 0;
	int buffer2_size = sizeof(encryptdatabuffer) - 1;
	// Receive data until encryptdatabuffer is filled
	while (index2 < sizeof(encryptdatabuffer) - 1) {
		readdata = receive(socketfd, encryptdatabuffer+index2, buffer2_size, 0);
        int check15;
		...
		index2 =index2+  readdata;
		buffer2_size =buffer2_size- readdata;
	}
	// Change the "@@" at the end of encryptdatabuffer into termination symbol
	int termposition = strstr(encryptdatabuffer, "@@") - encryptdatabuffer;
	encryptdatabuffer[termposition] = '\0';
